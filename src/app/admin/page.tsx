...
export default async function AdminDashboard() {
  ...
  const [{ count: staging }, { count: characters }, { count: animes }] = await Promise.all([
    supabase.from('staging_characters').select('*', { count: 'exact', head: true }),
    supabase.from('characters').select('*', { count: 'exact', head: true }),
    supabase.from('animes').select('*', { count: 'exact', head: true }).eq('is_active', true),
  ])

  const stats = [
    { label: 'En staging', value: staging ?? 0, color: 'text-yellow-400' },
    { label: 'Publiés', value: characters ?? 0, color: 'text-green-400' },
